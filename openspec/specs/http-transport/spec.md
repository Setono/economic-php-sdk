...
- **AND** the outgoing PSR-7 request carries `Content-Type: application/json` ONLY IF the request does not already carry a `Content-Type` header (a consumer-supplied value via `Client::request()` is preserved so the documented escape hatch for non-JSON endpoints actually works)
...
- **THEN** the auth headers and `User-Agent` are applied
- **AND** `Content-Type: application/json` is applied ONLY IF the request did not already carry a `Content-Type` header
- **AND** the response is journaled and dispatched through status-code mapping

#### Scenario: Consumer-supplied Content-Type is preserved

- **WHEN** the consumer builds a PSR-7 request with `Content-Type: application/pdf` and passes it to `Client::request()`
- **THEN** the outgoing request still carries `Content-Type: application/pdf` (the SDK does NOT overwrite it)
